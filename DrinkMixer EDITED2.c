...
const int LOWERDISTANCE = 16 * 360 / (2 * PI * 1.5); //note that 2.75 should be replaced with radius of wheel we're using
...
// Has user touch and release touch sensor before continuing
void touchConfirmation(){
	eraseDisplay();
	displayString(0,"Press the touch sensor to begin");
	while(SensorValue[S2] == 0){}
	while(SensorValue[S2] == 1){}
	eraseDisplay();
}

//Rotates motorD to the given position.  Modified for use with variables above
void rotate(int pos, int & motorPos)
{
	...
	{
		motor[motorD] = 5;
		while(nMotorEncoder[motorD] <= pos)
		{}
	}
...
//Calibrates motorD in the correct starting position (should run on startup and shutdown)
void calibrateMotorEnc(){
	...
	motor[motorD] = -5;
	while(nMotorEncoder[motorD] >= -133)
	{
		displayString(1, "%d", nMotorEncoder[motorD]);
	}
	motor[motorD] = 0;
	nMotorEncoder[motorD] = 0;
}
...
void drain(int container, int & motorPos){
	...
	if (getButtonPress(buttonEnter)) {
		...
		// Opens container valve for required amount of time plus three seconds to ensure complete drainage
		clearTimer(T1);
		setServoPosition(S1, container, 90);
		int drainTime = calculateTime(getDrinkLevel(container), container)+3000;
...
void stirDrink(int & motorPos) {
	...
	while(nMotorEncoder[motorB] < LOWERDISTANCE + 10)
	{}
...
void pourDrinks(int drinkSize, int drink1Amt, int drink2Amt, int & motorPos) {
	...
	if(drink1Level < drink1Amt) {
		...
		// Stirs drink if multiple drinks poured
		if (drink1Amt > 0 && drink2Amt > 0)
			stirDrink(motorPos);
